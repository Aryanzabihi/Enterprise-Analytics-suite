...
sys.path.extend([
    ...
    os.path.join(current_dir, 'invt'),
    os.path.join(current_dir, 'marketing'),
    os.path.join(current_dir, 'RD'),
    os.path.join(current_dir, 'sale')
])

# Add additional module paths for dependencies
additional_paths = [
    os.path.join(current_dir, 'cs'),  # For cs_metrics_calculator
    os.path.join(current_dir, 'pro'), # For metrics_calculator
    os.path.join(current_dir, 'hr'),  # For hr_predictive_analytics
    os.path.join(current_dir, 'invt'), # For invt_metrics_calculator
    os.path.join(current_dir, 'sale'), # For sales_metrics_calculator
    os.path.join(current_dir, 'marketing') # For marketing_metrics_calculator
]
...
# Department information and navigation - Ordered by Enterprise Priority
DEPARTMENTS = {
    'finance': {
    ...
    'inventory': {
        ...
        'icon': '📦',
        'description': 'Stock optimization, demand forecasting, supply chain analytics',
        'color': '#ff9a9e',
        'file': 'invt/invt.py',
        'module_name': 'invt'
    },
...
def run_department_app(dept_key):
    ...
    try:
        ...
        if hasattr(module, 'main') and callable(module.main):
            # Check if we're switching departments or if it's the first time
            previous_department = st.session_state.get('active_department')
            is_first_time = 'current_page' not in st.session_state
            is_switching_departments = previous_department != dept_key
            ...
            # Only set the department to start on home page if it's the first time or switching departments
            if is_first_time or is_switching_departments:
    ...
    if 'selected_department' in st.session_state and st.session_state.selected_department:
        ...
        <div class="footer">
            ...
            <div style="margin: 15px 0; text-align: center;">
                <a href="https://www.paypal.com/donate/?hosted_button_id=C9W46U77KNU9S" target="_blank" style="
                    display: inline-block;
